                    ...
                    {certifications.map((cert) => (
                        <div key={cert.org} className="border-b border-dark/10 pb-8 last:border-0 last:pb-0">
                            <div className="flex flex-col md:flex-row md:justify-between gap-1 mb-3">
                                <p className="text-dark/80 font-semibold text-base md:text-lg">{cert.org}</p>
                                <p className="text-dark/70 text-sm md:text-base shrink-0">{cert.location}</p>
                            </div>
                            <p className="text-dark/50 italic text-sm md:text-base mb-4">{cert.description}</p>
                            {cert.items.length > 0 && (
                                    ...
                                    {cert.items.map((item) => (
                                        <div key={item.name}>
                                            <div className="flex justify-between items-baseline mb-1">
                                                ...
                                                <p className="text-dark/50 text-sm shrink-0 ml-4">{item.period}</p>
                                            </div>
                                            {item.bullets.map((bullet, i) => (
                                                <li key={i} className="flex gap-2 text-dark/60 font-light text-sm md:text-base leading-relaxed list-none">
                            ...
                            {cert.period && (
                                <p className="text-dark/50 text-sm md:text-sm mt-1">{cert.period}</p>
                            )}
                        </div>
                    ))}
                </div>
