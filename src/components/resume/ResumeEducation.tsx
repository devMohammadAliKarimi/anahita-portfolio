                    ...
                    {education.map((item) => (
                        <div key={item.school} className="flex flex-col md:items-center md:flex-row md:justify-between gap-1 md:gap-4 border-b border-dark/10 pb-6 last:border-0 last:pb-0">
                            <div className="flex flex-col gap-1">
                                <p className="text-dark/80 font-semibold text-base md:text-lg">{item.school}</p>
                                <p className="text-dark/50 italic text-sm md:text-base">{item.degree}</p>
                            </div>
                            <div className="flex flex-col md:items-end gap-1 shrink-0">
                                <p className="text-dark/70 text-xs md:text-sm">{item.location}</p>
                                <p className="text-dark/50 text-xs md:text-sm">{item.period}</p>
                            </div>
                        </div>
                    ))}
                </div>
